...
function Lbl({ children }) {
  return (
    <div style={{
      color: "#445", fontSize: 11, fontWeight: 700, letterSpacing: 1.2,
...
function Inp(props) {
  return (
    <input {...props} style={{
      width: "100%", background: "#0c1018", border: "1px solid #222d3d",
      borderRadius: 8, color: "#ccd6f0", padding: "9px 13px", fontSize: 14,
      outline: "none", fontFamily: "inherit", boxSizing: "border-box", ...props.style
    }} />
  );
}
...
function Modal({ title, onClose, children }) {
  return (
    <div style={{
      ...
      <div style={{
        background: "#111720", border: "1px solid #222d3d", borderRadius: 18,
        padding: "26px 28px", width: "100%", maxWidth: 540,
...
function Spinner() {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "60vh", gap: 16 }}>
      <div style={{
        width: 48, height: 48, border: "4px solid #1a2030",
        borderTop: "4px solid #38c98a", borderRadius: "50%",
...
export default function App() {
  ...
  async function addPlayer() {
    ...
    try {
      ...
      toast("Error saving player. Please try again.");
    }
    setSaving(false);
  }
  ...
  async function logMatch() {
    ...
    const t1 = [nm.t1p1, nm.t1p2];
    const t2 = [nm.t2p1, nm.t2p2];
    try {
      // Save match
      await addDoc(collection(db, "matches"), {
        cat: nm.cat, t1p1: nm.t1p1, t1p2: nm.t1p2,
        t2p1: nm.t2p1, t2p2: nm.t2p2,
      ...
      const involved = players.filter(p =>
        t1.includes(p.name) || t2.includes(p.name)
      );
      ...
      toast("Error saving match. Please try again.");
    }
    setSaving(false);
  }
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#0a0d12", color: "#ccd6f0",
      fontFamily: "'Rajdhani','Barlow','Trebuchet MS',sans-serif",
      ...
      <header style={{
        ...
        padding: "14px 24px", borderBottom: `1px solid ${BDR}`,
        background: "#0c1018ee", backdropFilter: "blur(10px)",
        position: "sticky", top: 0, zIndex: 20
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
          <div style={{
            width: 40, height: 40, borderRadius: 10,
            background: "linear-gradient(135deg,#38c98a,#5b8af5)",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontWeight: 900, fontSize: 20, color: "#0a0d12"
          }}>S</div>
          <div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => setShowAP(true)} style={{
            background: "transparent", color: "#38c98a", border: "1px solid #38c98a44",
            borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 13,
            cursor: "pointer", fontFamily: "inherit"
          }}>＋ Player</button>
          <button onClick={() => setShowLM(true)} style={{
            background: "linear-gradient(90deg,#38c98a,#5b8af5)", color: "#0a0d12",
      ...
      {flash && (
        <div style={{
          ...
          background: "linear-gradient(90deg,#38c98a,#5b8af5)", color: "#0a0d12",
          padding: "9px 30px", borderRadius: 30, fontWeight: 800, zIndex: 999,
          fontSize: 13, boxShadow: "0 4px 28px #38c98a44", letterSpacing: .5,
          whiteSpace: "nowrap"
        }}>{flash}</div>
      )}
      ...
      <nav style={{ display: "flex", borderBottom: `1px solid ${BDR}`, background: "#0c1018", padding: "0 18px" }}>
        {[["leaderboard","🏆 Leaderboard"],["matches","📋 Matches"],["players","👤 Players"]].map(([t, lbl]) => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            color: tab === t ? "#38c98a" : "#334",
            padding: "13px 20px", fontSize: 14, cursor: "pointer",
            fontWeight: tab === t ? 800 : 700, letterSpacing: .4,
            fontFamily: "inherit",
            borderBottom: tab === t ? "2px solid #38c98a" : "2px solid transparent"
          }}>{lbl}</button>
        ))}
      </nav>
      ...
      <div style={{ padding: "22px 20px 0", maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14, alignItems: "center", marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                {[["ALL","All"],["MD","Men's Doubles"],["LD","Ladies' Doubles"],["XD","Mixed Doubles"]].map(([k, lbl]) => (
                  <button key={k} onClick={() => setFCat(k)} style={{
                    background: fCat === k ? (CAT[k]?.color || "#38c98a") : "#13182299",
                    color: fCat === k ? "#0a0d12" : "#667",
                    border: fCat === k ? "none" : `1px solid ${BDR}`,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <select value={fFlight} onChange={e => setFFlight(e.target.value)} style={{
                  background: "#13182299", border: `1px solid ${BDR}`, color: "#ccd6f0",
                  borderRadius: 8, padding: "5px 11px", fontSize: 12, fontFamily: "inherit", outline: "none"
                }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search player…" style={{
                marginLeft: "auto", background: "#13182299", border: `1px solid ${BDR}`,
                color: "#ccd6f0", borderRadius: 8, padding: "7px 13px",
                fontSize: 13, outline: "none", fontFamily: "inherit"
              }} />
            </div>

            {/* Flight overview pills */}
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 18 }}>
              {[...FLIGHTS].reverse().map(f => {
                const cnt = players.filter(p => p.flight === f.id).length;
                const active = fFlight === f.id;
                return (
                  <div key={f.id} onClick={() => setFFlight(fFlight === f.id ? "ALL" : f.id)} style={{
                    background: active ? f.glow : "#111720",
                    border: `1px solid ${active ? f.color : f.color + "33"}`,
            ...
            <div style={{ background: SRF, border: `1px solid ${BDR}`, borderRadius: 14, overflow: "auto", marginBottom: 28 }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
                ...
                <tbody>
                  {board.length === 0 && (
                    <tr><td colSpan={8} style={{ padding: 32, textAlign: "center", color: "#334", fontSize: 14 }}>
                  ...
                  {board.map((p, i) => {
                    ...
                    return (
                      <tr key={p.id} onClick={() => { setSelP(p); setTab("players"); }} style={{ borderBottom: `1px solid ${BDR}44`, cursor: "pointer" }}>
                        ...
                        <td style={{ padding: "11px 15px" }}>
                          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                            {cats.map(c => (
                              <span key={c} style={{ background: CAT[c].color + "22", color: CAT[c].color, borderRadius: 20, padding: "1px 8px", fontSize: 10, fontWeight: 800 }}>{c}</span>
                            ))}
                          </div>
                        </td>
        ...
        {tab === "matches" && (
          <div>
            ...
            {matches.length === 0 && (
              <div style={{ color: "#334", textAlign: "center", padding: 48, fontSize: 15 }}>No matches yet — log your first match!</div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {matches.map(m => {
                const cc = CAT[m.cat]?.color || "#888";
                const cl = CAT[m.cat]?.label || m.cat;
                return (
                  <div key={m.id} style={{ background: SRF, border: `1px solid ${BDR}`, borderRadius: 12, padding: "14px 18px", borderLeft: `4px solid ${cc}` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
                      <span style={{ background: cc + "22", color: cc, borderRadius: 20, padding: "2px 12px", fontSize: 11, fontWeight: 800 }}>{cl}</span>
                    ...
                    <div style={{ marginTop: 8, fontSize: 12, color: "#445" }}>
                      🏆 {m.winner === 1 ? `${m.t1p1} & ${m.t1p2}` : `${m.t2p1} & ${m.t2p2}`}
                    </div>
                  </div>
                );
              })}
        ...
        {tab === "players" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              ...
              {!selP && <button onClick={() => setShowAP(true)} style={{ background: "transparent", color: "#38c98a", border: "1px solid #38c98a44", borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>＋ Add Player</button>}
            </div>

            {selP ? <Profile p={selP} matches={matches} /> : (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(195px,1fr))", gap: 14, marginBottom: 28 }}>
                {players.length === 0 && (
                  <div style={{ color: "#334", gridColumn: "1/-1", textAlign: "center", padding: 48 }}>No players yet. Add your first player!</div>
                )}
                {players.map(p => {
                  const wr = p.matches > 0 ? Math.round(p.wins / p.matches * 100) : 0;
                  const cats = [p.md && "MD", p.ld && "LD", p.xd && "XD"].filter(Boolean);
                  return (
                    <div key={p.id} onClick={() => setSelP(p)} style={{ background: SRF, border: `1px solid ${BDR}`, borderRadius: 14, padding: "22px 16px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer" }}>
      ...
      {showAP && (
        <Modal title="Add New Player" onClose={() => setShowAP(false)}>
          ...
          <div style={{ display: "flex", gap: 20, marginTop: 4 }}>
            {[["M","Male"],["F","Female"]].map(([v, l]) => (
              <label key={v} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 14, color: "#889", cursor: "pointer" }}>
                <input type="radio" name="gender" checked={np.gender === v} onChange={() => setNp(x => ({ ...x, gender: v }))} /> {l}
              </label>
            ))}
          </div>
          ...
          <div style={{ fontSize: 12, color: "#445", marginTop: 10, lineHeight: 1.6 }}>
            Assign any starting flight based on known skill level. Flights adjust automatically as results are logged.
          </div>
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 20 }}>
            <button onClick={() => setShowAP(false)} style={{ background: "transparent", color: "#38c98a", border: "1px solid #38c98a44", borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
            <button onClick={addPlayer} disabled={saving} style={{ background: "linear-gradient(90deg,#38c98a,#5b8af5)", color: "#0a0d12", border: "none", borderRadius: 8, padding: "8px 20px", fontWeight: 800, fontSize: 13, cursor: saving ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: saving ? .7 : 1 }}>
        ...
        <Modal title="Log a Match" onClose={() => setShowLM(false)}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 4 }}>
            {Object.entries(CAT).map(([k, { label, color }]) => (
              <button key={k} onClick={() => setNm(v => ({ ...v, cat: k }))} style={{
                background: nm.cat === k ? color + "33" : "#13182299",
                color: nm.cat === k ? color : "#667",
                border: `1px solid ${nm.cat === k ? color : BDR}`,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 6 }}>
            {[[1,"Team 1",nm.t1p1,nm.t1p2],[2,"Team 2",nm.t2p1,nm.t2p2]].map(([val, lbl, p1, p2]) => (
              <label key={val} style={{
                ...
                color: nm.winner === val ? "#68c9a0" : "#889", cursor: "pointer",
                background: nm.winner === val ? "#68c9a011" : "transparent",
                border: `1px solid ${nm.winner === val ? "#68c9a044" : BDR}`,
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 22 }}>
            <button onClick={() => setShowLM(false)} style={{ background: "transparent", color: "#38c98a", border: "1px solid #38c98a44", borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
            <button onClick={logMatch} disabled={saving} style={{ background: "linear-gradient(90deg,#38c98a,#5b8af5)", color: "#0a0d12", border: "none", borderRadius: 8, padding: "8px 20px", fontWeight: 800, fontSize: 13, cursor: saving ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: saving ? .7 : 1 }}>
...
// ─── PROFILE ──────────────────────────────────────────────────────────────────
function Profile({ p, matches }) {
  const SRF = "#111720", BDR = "#1a2030";
  const wr = p.matches > 0 ? Math.round(p.wins / p.matches * 100) : 0;
  const cats = [p.md && "MD", p.ld && "LD", p.xd && "XD"].filter(Boolean);
